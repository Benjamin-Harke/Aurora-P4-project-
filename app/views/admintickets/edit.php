    ...
    <div class="col-xl-7 col-lg-9">
      ...
      <div class="card shadow-sm">
        <div class="card-header bg-dark text-white d-flex align-items-center gap-2">
          ...
          <h5 class="mb-0">Ticket Wijzigen — Ticket #<?= $data['ticket']->id; ?></h5>
        </div>
        ...
        <div class="card-body p-4">
          ...
          <form method="POST" action="<?= URLROOT; ?>/admintickets/edit/<?= $data['ticket']->id; ?>" novalidate>
            ...
            <div class="mb-4">
              ...
              <select class="form-select" id="status" name="status" required>
                <option value="Gereserveerd" <?= ($data['post']['status'] ?? '') == 'Gereserveerd' ? 'selected' : ''; ?>>Gereserveerd</option>
                <option value="booked" <?= ($data['post']['status'] ?? '') == 'booked' ? 'selected' : ''; ?>>Booked (Betaald)</option>
                <option value="Gescand" <?= ($data['post']['status'] ?? '') == 'Gescand' ? 'selected' : ''; ?>>Gescand</option>
                <option value="cancelled" <?= ($data['post']['status'] ?? '') == 'cancelled' ? 'selected' : ''; ?>>Cancelled (Geannuleerd)</option>
                <?php if (!in_array($data['post']['status'] ?? '', ['Gereserveerd', 'booked', 'Gescand', 'cancelled'])): ?>
                  <option value="<?= htmlspecialchars($data['post']['status'] ?? ''); ?>" selected>
                    <?= htmlspecialchars(ucfirst($data['post']['status'] ?? '')); ?>
                  </option>
                <?php endif; ?>
              </select>
            </div>
...
(function () {
  ...
  // Store original ticket settings to allow keeping the same seat
  const originalShowId     = "<?= $data['ticket']->voorstelling_id; ?>";
  const originalSeat       = parseInt("<?= $data['ticket']->nummer; ?>", 10);
  ...
  voorstellingSelect.addEventListener('change', function () {
    ...
    fetch('<?= URLROOT; ?>/admintickets/getSeats/' + id)
      ...
      .then(d => {
        ...
        // If the selected show is the original show, exclude the original seat from the display list
        let displayTakenSeats = [...takenSeats];
        if (id == originalShowId) {
          displayTakenSeats = displayTakenSeats.filter(s => s !== originalSeat);
        }

        totalSeatsEl.textContent     = totalSeats;
        availableSeatsEl.textContent = totalSeats - displayTakenSeats.length;
        takenSeatNrsEl.textContent   = displayTakenSeats.length > 0
          ? displayTakenSeats.sort((a, b) => a - b).join(', ')
          : 'Geen';
  ...
  function checkSeat() {
    ...
    const selectedShowId = voorstellingSelect.value;
    
    // Seat is taken if it's in takenSeats, UNLESS it's the original seat on the original performance
    const isOriginalSeat = (selectedShowId == originalShowId && val === originalSeat);
    
    if (val && takenSeats.includes(val) && !isOriginalSeat) {
  ...
  // ── If performance was pre-selected (initial load or error repopulation) ──
